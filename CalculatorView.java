 ...
 * @version May 7, 2019
 * 
 * Final Project - Matrix Calculator
 * 
 * The sole purpose of this class is to display the graphical user interface, also known
 *  as the GUI for the calculator. The class initializes buttons along with text boxes
 *  and links them together so the information can then be passed to the parser so the
 *  string can be taken apart into mathematical expressions.
 */
...
public class CalculatorView extends JPanel implements ActionListener, KeyListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == buttonCompute) {
			...
			inputWindow.setText(inputText + ".");
		}else if (e.getSource() == buttonAnswer){
			inputWindow.setText(inputText + answer);
	...
	private void cleanUp() {
		...
		buttonRemoveColumn.removeActionListener(this);
	...
	/*
	 * This method adds the window that is called whenever the button is pressed to add
	 *  a matrix. It starts off by drawing the matrix window with all of the buttons
	 *  and text boxes along with setting the editability of each of the matrix text
	 *  boxes. It finishes up by drawing the setting the window to visible so it can be
	 *  seen by the user.
	 */
	...
	/**
	 * This method draws the main window of the calculator itself. It adds all the buttons
	 *  and listeners to the interface in a way that makes it look most similar to 
	 *  resemble a calculator.
	 */
	...
	public CalculatorView() {
		...
		buttonPeriod.setToolTipText("Decimal Point");
		buttonAnswer.setToolTipText("Previous Answer");
	...
	public static CalculatorView createCalculator() {
		...
		CalculatorView newContentPane = new CalculatorView();
	...
	public void addComputeListener(ActionListener al) {
		buttonCompute.addActionListener(al);
	}
