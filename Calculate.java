public class Calculate extends Matrix {
    ...
    public Matrix inverse(Matrix m) {
    	...
    	if(determinant(m) == 0) {
    		throw new RuntimeException("Non-Invertable Matrix");
    	}
    	if(m.getRows() == 2) {
    		x = new int[2][2];
    		x[0][0] = (m.getNumber(1, 1)/m.getDeterminant());
    		x[0][1] = -(m.getNumber(0, 1)/m.getDeterminant());
    		x[1][0] = -(m.getNumber(1, 0)/m.getDeterminant());
    		x[1][1] = (m.getNumber(0, 0)/m.getDeterminant());
    		return new Matrix(x);
    	}
    	return new Matrix(0);
    }
